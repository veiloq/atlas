...
import (
	...
	arigaschema "ariga.io/atlas/sql/schema"
	"github.com/veiloq/atlas/pkg/migrate/ent"
...
func (r *EntRevisions) Migrate(ctx context.Context) (err error) {
	...
	case r.Dialect() != dialect.SQLite:
		...
		// opts = append(opts, entschema.WithSchemaName(r.schema)) // WithSchemaName not available in ent v0.13.1
	default: // SQLite.
		var on sql.NullBool
	...
	return c.Schema.Create(ctx, append(opts, entschema.WithDiffHook(func(next entschema.Differ) entschema.Differ {
		return entschema.DiffFunc(func(current, desired *arigaschema.Schema) ([]arigaschema.Change, error) {
			...
			var filteredChanges arigaschema.Changes
			for i := range changes {
				switch cs := changes[i].(type) {
				case *arigaschema.AddTable:
					// r.maySetSchemaQualifier(cs.T) // cs.T is *arigaschema.Table, function expects *schema.Table
					if cs.T.Name == revision.Table {
						filteredChanges = append(filteredChanges, cs)
					}
				case *arigaschema.ModifyTable:
					// r.maySetSchemaQualifier(cs.T) // cs.T is *arigaschema.Table, function expects *schema.Table
					if cs.T.Name == revision.Table {
						filteredChanges = append(filteredChanges, cs)
					}
			...
			// If we found specific revision table changes, return only those.
